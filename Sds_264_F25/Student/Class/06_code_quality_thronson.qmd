...
You can download this .qmd file from [here](https://github.com/proback/264_fall_2025/blob/main/06_code_quality.qmd).  Just hit the Download Raw File button.
...
vaccine_data |> 
  ...
  mutate(
    state_ordered = fct_reorder2(state, perc_Biden, perc_complete_vac),
    prop_Biden = perc_Biden / 100,
    prop_complete_vac = perc_complete_vac / 100
  ) |>
  ggplot(aes(x = prop_Biden, y = prop_complete_vac, color = state_ordered)) +
  geom_point() + 
  ...
  labs(
    ...
    title = "The positive relationship between Biden votes 
